...
{
	/// <summary>
	/// The SizeConverter class is used to convert from one data type to another.
	/// </summary>
	public class SizeDConverter : TypeConverter
	{
			...
			object value, Type destinationType) 
		{
			if (destinationType == typeof(string))
			{
				...
				else 
				{
					...
					return size.Width.ToString() + ch + size.Height.ToString();
				}
			}
			...
			if (destinationType == typeof(InstanceDescriptor) && value != null)
			{
				...
				ConstructorInfo info = typeof(SizeD).GetConstructor(types);
				if (info != null)
				{
		...
		public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
		{
			if (value is string)
			{
				...
				return new SizeD(double.Parse(words[0]), double.Parse(words[1]));
            }
